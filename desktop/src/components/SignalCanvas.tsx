...
function AnalysisStrip({ selected }: { selected: Opportunity }) {
  ...
  return <><div className="analysis-strip"><span><small>EDGE</small><strong>{analysis.opportunity_score}</strong></span><span><small>P(TP&gt;SL)</small><strong>{(Number(analysis.p_tp_before_sl) * 100).toFixed(1)}%</strong></span><span><small>EV</small><strong className={Number(analysis.expected_value) > 0 ? "positive" : "negative"}>{analysis.expected_value}</strong></span><span><small>{analysis.market_regime}</small><strong>{analysis.signal_type}</strong></span><div className="analysis-biases"><span>SMART {analysis.smart_money_bias}</span><span>DERIVATIVES {analysis.derivatives_bias}</span><span>FLOW {analysis.order_flow_bias}</span><span>NEWS {analysis.news_bias}</span></div></div>{decision && <div className={`decision-panel ${planExpired ? "no_trade" : decision.outcome.toLowerCase()}`}><strong>{planExpired ? "暂不交易 · 执行窗口已结束" : `${decision.outcome === "TRADE" ? "可交易" : "暂不交易"} · ${passed}/${decision.gates.length} 门槛通过`}</strong>{!planExpired && <div>{decision.gates.map((gate) => <span key={gate.key} aria-label={gate.label} className={gate.passed ? "passed" : "blocked"}>{gate.passed ? "✓" : "×"} {gate.label}</span>)}</div>}{calibrationText && <small className="calibration-note">{calibrationText}</small>}</div>}</>;
}
