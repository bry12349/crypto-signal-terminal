...
function AnalysisStrip({ selected }: { selected: Opportunity }) {
  ...
  const planExpiry = selected.order_plan ? Date.parse(selected.order_plan.expires_at) : Number.NaN;
  const planExpired = Boolean(selected.order_plan && (!Number.isFinite(planExpiry) || planExpiry <= Date.now()));
  ...
  return <><div className="analysis-strip"><span><small>EDGE</small><strong>{analysis.opportunity_score}</strong></span><span><small>P(TP&gt;SL)</small><strong>{(Number(analysis.p_tp_before_sl) * 100).toFixed(1)}%</strong></span><span><small>EV</small><strong className={Number(analysis.expected_value) > 0 ? "positive" : "negative"}>{analysis.expected_value}</strong></span><span><small>{analysis.asset_profile ?? analysis.market_regime} · v{analysis.model_version ?? "0.6"}</small><strong>{analysis.signal_type}</strong></span><div className="analysis-biases"><span>SMART {analysis.smart_money_bias}</span><span>DERIVATIVES {analysis.derivatives_bias}</span><span>FLOW {analysis.order_flow_bias}</span><span title={analysis.narrative_sources?.join(" · ")}>NARRATIVE {analysis.narrative_bias ?? analysis.news_bias}</span></div></div>{decision && <div className={`decision-panel ${planExpired ? "no_trade" : decision.outcome.toLowerCase()}`}><strong>{planExpired ? "暂不交易 · 执行窗口已结束" : `${decision.outcome === "TRADE" ? "可交易" : "暂不交易"} · ${passed}/${decision.gates.length} 门槛通过`}</strong>{!planExpired && <div>{decision.gates.map((gate) => <span key={gate.key} aria-label={gate.label} className={gate.passed ? "passed" : "blocked"}>{gate.passed ? "✓" : "×"} {gate.label}</span>)}</div>}{calibrationText && <small className="calibration-note">{calibrationText}</small>}</div>}</>;
}
...
export function SignalCanvas({ selected, candles, mode, health }: { selected: Opportunity | null; candles: Candle[]; mode: string; health: MarketSymbolHealth | undefined }) {
  ...
  const planExpiry = selected.order_plan ? Date.parse(selected.order_plan.expires_at) : Number.NaN;
  const planExpired = Boolean(selected.order_plan && (!Number.isFinite(planExpiry) || planExpiry <= Date.now()));
  const executable = Boolean(selected.order_plan && !planExpired && selected.state === "ENTRY_VALID" && selected.analysis?.is_tradeable !== false);
  const direction = executable ? selected.order_plan?.direction : undefined; const base = isOnchainChart ? selected.symbol : marketSymbol.replace("USDT", "");
  ...
  const executionWindowLabel = planExpired ? "信号已过期" : executable ? "有效窗口开启" : selected.analysis?.is_tradeable === false ? "风控未通过" : "等待触发";
  const profile = selected.analysis?.asset_profile;
  const contract = isOnchainChart ? "BSC DEX" : profile === "COMMODITY" ? "COMMODITY PERP" : profile === "US_EQUITY" ? "US STOCK PERP" : "USDT PERP";
  const tokenIdentity = onchainTokenAddress ? `${onchainTokenAddress.slice(0, 8)}…${onchainTokenAddress.slice(-6)}` : null;
  return <main className="signal-canvas"><div className="instrument-head"><div><span className="eyebrow">SELECTED INSTRUMENT · UTC+8</span><h1>{base}<small> / {contract}</small></h1>{tokenIdentity && <span className="instrument-token-id">Token {tokenIdentity}</span>}</div><div className={`direction-lock ${direction?.toLowerCase() ?? "watch"}`}><span>{direction ?? "NO TRADE"}</span><strong>{selected.confidence}</strong><small>{selected.state === "FORMING" ? "观察强度" : "结构可信度"}</small></div></div>
    <div className="setup-ribbon"><span><Layers3 size={14} />{selected.title}</span><span><Clock3 size={14} />{executionWindowLabel}</span><span className={`data-fresh ${!isOnchainChart && health?.status !== "healthy" ? "unhealthy" : ""}`}>{isOnchainChart || health?.status === "healthy" ? <CheckCircle2 size={14} /> : <AlertTriangle size={14} />}{healthLabel}</span></div>
    ...
    {(isWalletFlow || selected.analysis) && <section className="evidence-panel technical-panel">{isWalletFlow && <div className="wallet-flow-summary"><div><span className="eyebrow">ON-CHAIN SMART MONEY</span><h3>钱包流向详情</h3></div><span className="wallet-market-benchmark">{isOnchainChart ? `链上池 · ${base} / BSC` : `市场基准 · ${base}/USDT`}</span><p>{selected.source_label}</p><small>{isOnchainChart ? "K 线来自 BSC DEX 的公开流动性池；钱包活动不等同于中心化交易所的合约开仓。" : "当前 K 线仅用于市场环境确认；钱包活动不等同于中心化交易所的合约开仓。"}</small></div>}<AnalysisStrip selected={selected} /></section>}
  </main>;
}
